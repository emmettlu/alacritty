...
macro_rules! impl_replace {
    ($($ty:ty),*$(,)*) => {
        $(
            impl $crate::config_compat::SerdeReplace for $ty {
                fn replace(&mut self, value: toml::Value) -> Result<(), Box<dyn std::error::Error>> {
                    $crate::config_compat::replace_simple(self, value)
                }
            }
        )*
    };
...
pub(crate) fn replace_simple<'de, D>(data: &mut D, value: Value) -> Result<(), Box<dyn Error>>
where
    D: Deserialize<'de>,
{
